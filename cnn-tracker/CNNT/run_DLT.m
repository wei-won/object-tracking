...
function results=run_DLT(seq, res_path, bSaveImage)
    ...
    for f = 1:size(seq.s_frames,1)  
      ...
      if  paramp1.update
          ...
          %opts.alpha = 1;
          nn1 = cnntrain(nn1, reshape([pos1 neg],[32,32,110]), [ones(opt.maxbasis + 1, 1); zeros(99, 1)]', opts);
      end
      if  paramp2.update
          opts.batchsize = 10;
          % Sample two set of negative samples at different range.
          ...
          nn2 = cnntrain(nn2, reshape([pos2 neg],[32,32,110]), [ones(opt.maxbasis + 1, 1); zeros(99, 1)]', opts);
      end
      ...
      if  paramp3.update
          ...
          nn3 = cnntrain(nn3, reshape([pos3 neg],[32,32,110]), [ones(opt.maxbasis + 1, 1); zeros(99, 1)]', opts);
      end
