...
if useGpu
    esn.inputWeights = gpuArray.rand(nInternalUnits, nInputUnits)*2.0-1.0;
else
    esn.inputWeights = 2.0 * rand(nInternalUnits, nInputUnits)- 1.0;
end
...
%     if useGpu
%         esn.inputScaling = gpuArray.ones(nInputUnits, 1);
%         esn.inputShift = gpuArray.zeros(nInputUnits, 1);
%     else
...
%     if useGpu
%         esn.inputScaling = gpuArray([]);
%         esn.inputShift = gpuArray([]);
%     else
%         esn.inputScaling = [];
%         esn.inputShift = [];
%     end
...
if useGpu
    esn.inputWeights = gpuArray(esn.inputWeights);
    esn.internalWeights_UnitSR = gpuArray(esn.internalWeights_UnitSR);
    esn.inputScaling = gpuArray(esn.inputScaling);
end
