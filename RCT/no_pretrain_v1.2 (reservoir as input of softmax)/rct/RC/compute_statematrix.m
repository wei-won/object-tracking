...
if useGpu
    stateCollectMat = gpuArray(stateCollectMat);
end
%% set starting state
...
if externalStartStateFlag == 0
    totalstate = zeros(esn.nInputUnits + esn.nInternalUnits, 1);
%     totalstate = zeros(esn.nInputUnits + esn.nInternalUnits + esn.nOutputUnits, 1);
    internalState = zeros(esn.nInternalUnits, 1);
end

if useGpu
    totalstate = gpuArray(totalstate);
    internalState = gpuArray(internalState);
end
