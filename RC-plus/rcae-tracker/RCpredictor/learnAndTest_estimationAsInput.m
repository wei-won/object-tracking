...
inputCollection = zeros(inputLength,samplelength);
firstIn = sampleinput(:,1);
inputCollection(:,1) = firstIn;

for i = 1:initialRunlength + sampleRunlength + freeRunlength + plotRunlength 
    
    if inputLength > 0
        in = [diag(inputscaling) * inputCollection(:,i) + inputshift];  % in is column vector  
    else in = [];
    end
...
% plot overlay of network output and teacher
figure(fig6);
plot(teacherPL(1,:),teacherPL(2,:));
hold on;
plot(netOutPL(1,:),netOutPL(2,:));
hold off;
